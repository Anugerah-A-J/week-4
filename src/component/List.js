  ...
  return (
    <div>
      ...
      <div className="list-wrapper mt-3">
        <input className='search' type="text" placeholder='search here...' onChange={(event) => setSearch(event.target.value)} />
        <div className="row mt-3">
          {places?.filter(item => item.siteName.toLowerCase().includes(search) ||
            item.siteLocation.toLowerCase().includes(search) ||
